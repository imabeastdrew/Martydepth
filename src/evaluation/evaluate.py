...
def load_model_from_wandb(artifact_path: str, device: torch.device):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Look for online transformer model files
        model_files = list(artifact_path.glob("online_transformer_epoch_*.pth"))
        if not model_files:
        ...
        print(f"Loading online model from: {model_path.name}")
